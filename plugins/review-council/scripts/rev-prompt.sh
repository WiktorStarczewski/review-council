...
}
{
  ...
  if [ "${REV_SEAT_OFFLINE:-}" = 1 ]; then
    echo "## Offline review"
    if [ -n "${REV_DEPS_DIR:-}" ]; then echo "Judge only what is in the repository above and in the pinned third-party dependency sources linked under $REV_DEPS_DIR (one directory per pinned crate, taken from the lockfile); that view is the only dependency source you may read — never the cargo registry itself, which also holds other versions of this repository'"'"'s own crates."; else
    echo "Judge only what is in the repository above and in the pinned dependency sources already on this machine (the cargo registry, node_modules)."; fi; echo " Do not use the network, web search, package downloads, or any other checkout of this repository on this machine; do not fetch, and do not read published versions of packages that this repository itself publishes — they may postdate the change — and do not search the whole registry; open only the specific crates or packages this repository pins. Do not read build directories outside the repository (a global CARGO_TARGET_DIR, another checkout\'s target/ or node_modules/): they hold artifacts of other states of this code. If you cannot establish something from those sources, say so instead of looking it up."; echo
  fi
  if [ -n "$PLAN" ]; then
