...
#   - it has no schema flag and no reasoning-effort knob. The prompt (which carries the schema) goes
#     on stdin and `-p` carries only the standing instruction; stream-summary.py lifts the outermost
