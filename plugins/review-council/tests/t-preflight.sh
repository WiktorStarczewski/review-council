...
# Seats no longer come from hardcoded provider checks: preflight asks
...
{"generated_at": "2026-09-02T00:00:00Z", "seats": [
 ...
 {"seat": "sonnet", "lab": "anthropic", "adapter": "claude", "model": "sonnet", "effort": "max", "extra": false},
...
[ -n "$WRITE" ] && printf '%s\n' "$JSON" > "$WRITE"
if [ "$BRIEF" = 1 ]; then
  printf '%s\n' "${RSTUB_BRIEF:-review-council seats: codex ✓ (gpt-5.6-sol@max, gpt-5.6-terra@max) · gemini ✗ not installed · claude ✓ (opus@max, sonnet@max)}"
else
...
{"generated_at": "2026-09-02T00:00:00Z", "seats": [
 {"seat": "codex-terra", "lab": "openai", "adapter": "codex", "model": "gpt-5.6-terra", "effort": "max", "extra": false},
 {"seat": "opus", "lab": "anthropic", "adapter": "claude", "model": "opus", "effort": "max", "extra": false},
 {"seat": "opus-1", "lab": "anthropic", "adapter": "claude", "model": "opus", "effort": "max", "extra": false, "padded": true}],
 "labs": ["openai", "anthropic"], "padded": 1, "degraded": true,
 "degradation": "available labs: openai, anthropic - 1 repeat CLI seats; reduced panel diversity",
 "result_receipts": {"version": 1, "legacy_no_exit_sha256": {}},
 "excluded": [{"cli": "codex", "reason": "not signed in"}, {"cli": "gemini", "reason": "not installed"}]}
J
}
...
test_preflight() {
  ...
  local contract_runner="$T/contract-runner"
  cat > "$contract_runner" <<'SH'
#!/bin/bash
printf '%s\n' "$1" >> "$CONTRACT_CALLS"
exit "${CONTRACT_EXIT:-0}"
SH
  chmod +x "$contract_runner"
  mkdir -p "$R/plugins/review-council/scripts" "$R/plugins/review-council/.codex-plugin"
  ...
  REVIEW_COUNCIL_CACHE_DIR="$T/contract-cache" CONTRACT_CALLS="$T/contract.calls" CONTRACT_EXIT=2 \
  ...
  REVIEW_COUNCIL_CONTRACT_RUNNER="$contract_runner" \
    REVIEW_COUNCIL_CONTRACT_PROVIDER_VERSIONS='{"codex":"1","claude":"1"}' \
    REVIEW_COUNCIL_CACHE_DIR="$T/contract-cache" CONTRACT_CALLS="$T/contract.calls" \
    "$PF" --write "$T/pf-contract-pass" > "$T/pf.out" 2> "$T/pf.err"
  assert_eq "passing provider replay allows preflight" "$?" 0
  assert_eq "provider replay runs every contract group" \
    "$(wc -l < "$T/contract.calls" | tr -d ' ')" "4"
  assert_grep "provider replay includes preserved provider envelopes" \
    "$T/contract.calls" '^provider_envelope_replay$'
  assert_eq "provider replay writes one immutable keyed receipt" \
    "$(find "$T/pf-contract-pass" -name 'contract-pass-*.json' | wc -l | tr -d ' ')" "1"
  python3 - "$T/pf-contract-pass" <<'PY'
import json, pathlib, sys
session = pathlib.Path(sys.argv[1])
receipt = json.loads(next(session.glob('contract-pass-*.json')).read_text())
assert receipt['identity']['core_roster'][0] == {
    'seat':'codex-sol','adapter':'codex','model':'gpt-5.6-sol','effort':'max'}
assert receipt['identity']['executor']['plugin'] != str(pathlib.Path.cwd() / 'plugins/review-council')
assert receipt['identity']['executor']['policy'] == 'external-override'
assert receipt['identity']['executor']['runner_sha256']
PY
  assert_eq "preflight contract identity uses the probed roster" "$?" 0
  REVIEW_COUNCIL_CONTRACT_RUNNER="$contract_runner" \
    REVIEW_COUNCIL_CONTRACT_PROVIDER_VERSIONS='{"codex":"1","claude":"1"}' \
    REVIEW_COUNCIL_CACHE_DIR="$T/contract-cache" CONTRACT_CALLS="$T/contract.calls" \
    "$PF" --write "$T/pf-contract-cached" > "$T/pf.out" 2> "$T/pf.err"
  assert_eq "matching provider contract receipt is reusable" "$?" 0
  assert_eq "cached provider replay launches no contract group" \
    "$(wc -l < "$T/contract.calls" | tr -d ' ')" "4"
  rm "$R/plugins/review-council/scripts/rev-prompt.sh" \
    "$R/plugins/review-council/.codex-plugin/plugin.json"
...
sonnet
EOF
  assert_exit "preflight passes quota handoffs exactly once and in order" 0 \
    cmp -s "$T/pf-quota-expected.args" "$RSTUB_ARGS"
  : > "$RSTUB_ARGS"
  "$PF" --quota-failed-seat > "$T/pf.out" 2> "$T/pf.err"
  assert_eq "preflight refuses a missing quota handoff seat" "$?" 1
  assert_exit "invalid handoff does not reach the roster" 0 test ! -s "$RSTUB_ARGS"
...
SH
  ...
  RSTUB_JSON="$(pf_degraded_roster)" \
    RSTUB_BRIEF='review-council seats: codex ✓ (gpt-5.6-terra@max) · gemini ✗ not installed · claude ✓ (opus@max) · DEGRADED: available labs: openai, anthropic - 1 repeat CLI seats; reduced panel diversity' \
  ...
  assert_grep "the roster line carries DEGRADED" "$T/pf.out" 'DEGRADED: available labs: openai, anthropic - 1 repeat CLI seats; reduced panel diversity$'
  assert_grep "…and a warning line of its own follows it" "$T/pf.out" \
    '^preflight: WARNING - available labs: openai, anthropic - 1 repeat CLI seats; reduced panel diversity$'
  ...
  local D2="$T/pf-bin-noroster"; mkdir -p "$D2"
  ln -sf "$SCRIPTS/rev-preflight.sh" "$D2/rev-preflight.sh"
  ln -sf "$SCRIPTS/rev-contract-check.py" "$D2/rev-contract-check.py"
...
test_prompt() {
  ...
  printf '%s' '{"seats":[{"seat":"codex-sol","adapter":"codex"},{"seat":"codex-terra","adapter":"codex"},{"seat":"opus","adapter":"claude"},{"seat":"sonnet","adapter":"claude"}]}' > "$S/roster.json"
  local p; p=$("$SCRIPTS/rev-prompt.sh" "$S" 1 codex-terra correctness "Correctness, edge cases, error handling")
  assert_eq "prints path" "$p" "$S/r1-codex-terra.prompt.md"
  assert_grep "Terra begins with assigned-hunk discovery" "$p" 'begin with assigned-hunk discovery'
  ...
  p=$("$SCRIPTS/rev-prompt.sh" "$S" 5 codex-terra correctness "Untracked")
  ...
  p=$("$SCRIPTS/rev-prompt.sh" "$S" 1 codex-terra correctness "Prose review" --read-only "$S/docs.txt")
  ...
  p=$("$SCRIPTS/rev-prompt.sh" "$S" 4 codex-terra correctness "Injection") ; assert_eq "space/metachar scope.env ok" "$?" 0
  ...
  printf 'REV_BASE=abc123\nREV_BRANCH=feat\nREV_DEFAULT=main\nREV_ROOT=/repo\nREV_SCOPE=branch\n' > "$S/scope.env"
  : > "$S/empty-docs.txt"
  assert_exit "empty --read-only list → 1" 1 "$SCRIPTS/rev-prompt.sh" "$S" 1 codex-terra correctness x --read-only "$S/empty-docs.txt"
  assert_exit "missing --read-only list → 1" 1 "$SCRIPTS/rev-prompt.sh" "$S" 1 codex-terra correctness x --read-only "$S/no-such-list.txt"
  "$SCRIPTS/rev-prompt.sh" "$S" 1 codex-terra correctness x --read-only "$S/empty-docs.txt" 2> "$T/ro.err"
  assert_grep "names the missing list" "$T/ro.err" 'document list missing or empty'
  assert_exit "unknown flag → 1" 1 "$SCRIPTS/rev-prompt.sh" "$S" 1 codex-terra correctness x --bogus
  # unwritable output must fail loudly, not print a path for a file that was never written
  assert_exit "unwritable OUT → 1" 1 "$SCRIPTS/rev-prompt.sh" "$T/no-such-session" 1 codex-terra correctness x --read-only "$S/docs.txt"
  printf 'REV_ROOT=/repo\n' > "$S/scope.env"
  assert_exit "incomplete scope.env → 1" 1 "$SCRIPTS/rev-prompt.sh" "$S" 1 codex-terra correctness x
  rm "$S/scope.env"; assert_exit "missing scope.env → 1" 1 "$SCRIPTS/rev-prompt.sh" "$S" 1 codex-terra correctness x
}
