...
DUPLICATE_TESTS=$(awk '
  /^[[:space:]]*function[[:space:]]+test_[A-Za-z0-9_]+/ ||
  /^[[:space:]]*test_[A-Za-z0-9_]+[[:space:]]*\(\)/ {
    match($0, /test_[A-Za-z0-9_]+/)
    name = substr($0, RSTART, RLENGTH)
    count[name]++
    if (count[name] == 2) print name
  }
' "${TEST_FILES[@]}")
if [ -n "$DUPLICATE_TESTS" ]; then
  while IFS= read -r name; do
    echo "run-tests: duplicate test function: $name" >&2
  done <<< "$DUPLICATE_TESTS"
  exit 1
fi
