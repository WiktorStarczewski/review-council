...
test_adaptive_schedule_contract() {
  ...
  for K in "$SK/skills/rev/SKILL.md" "$SK/codex-skills/rev/SKILL.md"; do
    ...
    assert_grep "all-lens plan panel restores every surviving core seat" "$K" '"plan_seats": "all"`, set `PLAN_SEATS` to every surviving non-extra seat'
    assert_grep "default plan panel is one non-agent seat" "$K" '`PLAN_SEATS` to the first surviving non-extra seat in roster order whose adapter is not `agent`'
