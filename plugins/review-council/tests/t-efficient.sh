...
test_adaptive_schedule_contract() {
  ...
  for K in "$SK/skills/rev/SKILL.md" "$SK/codex-skills/rev/SKILL.md"; do
    ...
    assert_flat_fixed "assignment replacement is canonical" "$K" "$repair"
    ...
    assert_flat_fixed "adaptive completion covers every bundle" "$K" 'A certified risk or verification panel therefore has all four bundles.'
    ...
    assert_grep "replacement launch records its exact seat" "$K" 'phase=repair round=<N>x "seats=\[\\"\$EXECUTOR\\"\]"'
